...
export class NodeLoader {
  ...
  async load(
    ...
    onNodeLoaded?: (node: OctreeGeometryNode) => void
  ) {
    ...
    const preData = await this.nodeDecorator.prepareData(node.name);
    ...
    try {
      ...
      if (byteSize === BigInt(0)) {
        ...
        let response = await this.requestManager.fetch(
          ResourceTarget.OCTREE_BIN,
          {
            headers: {
      ...
      worker.onmessage = (e) => {
        ...
        for (let property in buffers) {
          ...
          if (property === "position") {
            ...
            this.nodeDecorator.decorateRgbaBufferAttribute(rgba, preData);
        ...
        this.nodeDecorator.decorateBufferGeometry(geometry, preData);
