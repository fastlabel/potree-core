...
export function generateClassificationTexture(
...
export function generateVisibleTexture(visibleArray: boolean[]): Texture {
  const count = visibleArray.length;
  // 正方形にする
  const size = Math.ceil(Math.sqrt(count));
  const values = new Float32Array(size * size);

  // ★ 元の配列を texture にコピー（余りは 0）
  for (let i = 0; i < count; i++) {
    values[i] =
    visibleArray[i] === undefined || visibleArray[i] === true ? 1.0 : 0.0;
  }

  const texture = new DataTexture(values, size, size, RedFormat, FloatType);
